...
type FD = {
  email: string; emailOptional: string; firstName: string; surname: string;
  location: string; phone: string; searchServiceLevel: string; mainReason: string;
  searchFactors: string[]; preferredLocation: string; budget: string;
  horseAgeRange: string[]; horseHeight: string[]; characteristicsLiked: string;
  dealBreakers: string; mainDiscipline: string; horseType: string; riderGoals: string;
  currentCompetitionLevel: string; futureGoals: string; riderCompetence: string;
  ridingConfidence: string; riderHistory: string; riderAge: string;
  horseStatements: string[]; horseManagement: string[]; searchRestrictions: string[];
  otherInfo: string; termsAgreed: boolean; readyToSign: string; signature: string;
};
  ...
  return (
    <div className="min-h-screen bg-stone-50">
      ...
      <div className="max-w-2xl mx-auto px-4 py-8 space-y-6">
        ...
        {/* Intro */}
        <div className="bg-sky-50 border border-sky-200 rounded-xl p-5 text-sm text-stone-700 space-y-3 leading-relaxed">
        ...
        {/* Section 2 — Search Service */}
        <div className="bg-white rounded-2xl border border-stone-200 shadow-sm p-6 space-y-5">
          <SectionHeader title="Search Service Selection" />
          <label className={`block rounded-xl border-2 p-4 cursor-pointer transition-all ${s("searchServiceLevel") === "level2" ? "border-[#24384e] bg-[#24384e]/5" : "border-stone-200 hover:border-stone-300"}`}>
            <div className="flex items-start gap-3">
              <input type="radio" name="searchServiceLevel" value="level2"
                checked={s("searchServiceLevel") === "level2"}
                onChange={() => set("searchServiceLevel")("level2")}
                className="mt-1 h-4 w-4 accent-[#24384e] shrink-0" />
              <div>
                <p className="font-semibold text-stone-900 text-sm">Premium 'Concierge' Search</p>
                <p className="text-xs text-amber-700 font-medium mt-0.5">$1,000 upfront PLUS 5% Consultancy Fee (min $1,000, capped at $2,000)</p>
                <p className="text-xs text-stone-500 mt-1.5 leading-relaxed">Perfect if you don't have the time or skills to find and successfully secure the horse of your dreams. Includes all Standard inclusions PLUS managing all initial enquiries, negotiation, booking viewings, vet check guidance, transport contacts, and a Bill of Sale after completion.</p>
              </div>
            </div>
          </label>
        ...
        {/* Section 6 — Discipline & Horse Type */}
        <div className="bg-white rounded-2xl border border-stone-200 shadow-sm p-6 space-y-5">
        ...
        <div className="bg-white rounded-2xl border border-stone-200 shadow-sm p-6 space-y-5">
          ...
          <div>
            <FieldLabel label="I would like to restrict the search by the following"
        ...
        <div className="pb-8">
          <button onClick={submit} disabled={submitting}
            className="w-full px-6 py-3.5 rounded-xl bg-amber-700 text-white text-sm font-semibold hover:bg-amber-800 disabled:opacity-50 transition-colors shadow-sm">
