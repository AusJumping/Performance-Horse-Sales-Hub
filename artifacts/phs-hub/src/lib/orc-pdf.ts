...
export function generateOrcDriveHtml(data: OrcDocumentData): string {
  ...
  if (data.sellerName) metaParts.push(`Seller: ${escapeHtml(data.sellerName)}`);
  if (data.askingPrice) metaParts.push(`Asking: ${escapeHtml(data.askingPrice)}`);
...
<body style="font-family:Arial,sans-serif;max-width:800px;margin:0 auto;padding:24px;color:#222">
  ...
  <p style="font-size:10px;color:#aaa;margin:0">Generated by PHS App on ${dateStr}. Performance Horse Sales — Confidential, Internal Use Only.</p>
