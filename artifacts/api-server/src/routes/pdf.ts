...
// ── Exported buffer builder (used by email alerts) ────────────────────────────

type SubmissionRow = typeof submissionsTable.$inferSelect;
type MediaRow = typeof mediaFilesTable.$inferSelect;
...
export async function buildSubmissionPdfBuffer(
  ...
  return new Promise((resolve, reject) => {
    ...
    doc.moveTo(PAGE_MARGIN, doc.y + 4).lineTo(doc.page.width - PAGE_MARGIN, doc.y + 4)
    ...
    for (const section of FORM_SECTIONS) {
      ...
      for (const field of fieldsWithData) {
        ...
        if (field.key === "signature" && strVal.startsWith("data:image")) {
          ...
          doc.fontSize(8).font("Helvetica-Bold").fillColor(NAVY).text("Signature", PAGE_MARGIN, doc.y);
          doc.moveDown(0.3);
    ...
    const extraFields = Object.entries(formData).filter(
      ([k, v]) => !KNOWN_KEYS.has(k) && v !== null && v !== undefined && String(v).trim() !== "" && !k.startsWith("_")
    );
    ...
    const videoLinks = String(formData["videoLinks"] || "").split("\n").map((l) => l.trim()).filter(Boolean);
    if (videoLinks.length > 0) {
      drawSectionHeader(doc, "Video Links (Provided by Seller)");
      for (const link of videoLinks) drawField(doc, "Link", link, { wide: true });
    }
    ...
    if (media.length > 0) {
      ...
      if (photos.length > 0) drawField(doc, `Photos (${photos.length})`, photos.map((m) => m.originalName).join(", "), { wide: true });
      if (videos.length > 0) drawField(doc, `Videos (${videos.length})`, videos.map((m) => m.originalName).join(", "), { wide: true });
      if (docs.length > 0)   drawField(doc, `Documents (${docs.length})`, docs.map((m) => m.originalName).join(", "), { wide: true });
    }
    ...
    for (let i = range.start; i < range.start + range.count; i++) {
      ...
      doc.fillColor(NAVY_LIGHT).fontSize(7).font("Helvetica").text(
...
router.get("/submissions/:id/pdf", async (req, res) => {
  ...
  const media = await db.select().from(mediaFilesTable).where(eq(mediaFilesTable.submissionId, id));

  const formData = (submission.formData ?? {}) as Record<string, unknown>;
  const clientName = submission.sellerName ?? [formData["firstName"], formData["secondName"]].filter(Boolean).join(" ") ?? "Client";
  const horseName  = submission.horseName ?? (formData["horseName"] as string) ?? "Horse";
  const safeName   = `${sanitiseFilename(clientName)}_${sanitiseFilename(horseName)}_Submission.pdf`;

  const buf = await buildSubmissionPdfBuffer(submission, media);
